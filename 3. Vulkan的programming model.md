...
![WSI将image在Display和application之间轮换](images/3/3.png)
...
3. sparse memory的支持
	...
	> 基于sparse memory的方法会影响效率，写入时切分小块，读出时再组合，有额外的工作。

4. 使用staging buffer作为数据中转站

	如果要将vertex等data放到内存中，最简单的方法是直接放到CPU中，即host loca。但是吗，前面说到，这种类型的memory的特点很慢，当都需要读取大量数据时，受限于带宽，无法实现最优的性能，因此将数据放到device local才是最理想的方式。基于此，需要首先在CPU上构建staging buffer，将数据传输到这里，然后再将这些数据转移到device local中，[vulkan-tutorial中有讲这个过程](https://vulkan-tutorial.com/Vertex_buffers/Vertex_input_description)。
...
- pipeline layout
	...
	该对象描述了给定一个pipeline object，shaders的输入信息是什么样的。具体来说，就是通过descriptor sets，将resources与shaders建立对应关系通过binding slot，以便后续shaders能够读取相应的数据，完成drawing tasks。
...
这一步就是收集各个commands的过程，以便后续提交到queue中。为了记录application提交的commands，需要将它们包裹在一定的范围内，如下图是一个drawing command buffer被记录的描述：
